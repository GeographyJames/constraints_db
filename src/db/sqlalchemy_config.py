...
def credentials_from_ini(db_credentials: Path, password: str | None = None) -> DbCredentiails:
    ...
    if not db_credentials.is_file():
        raise CredentialsError(f"Unable to locate credentials .ini file ({db_credentials}). Check file path is valid file.")
    ...
    if "password" not in db_dict.keys():
        ...
        if not password:
            raise CredentialsError(f"No password provided. No environment variable set for {postgres_env_varialbl_name}.")
